...
func (a *Args) Handler(ctx context.Context) error {
	...
	if err := cluster1.EnforceForeignCluster(ctx, cluster2.GetClusterID(), cluster2.GetAuthToken(), cluster2.GetAuthURL(), cluster2.GetProxyURL()); err != nil {
		return err
	}

	// Creating foreign cluster in cluster2 for cluster 1.
	if err := cluster2.EnforceForeignCluster(ctx, cluster1.GetClusterID(), cluster1.GetAuthToken(), cluster1.GetAuthURL(), cluster1.GetProxyURL()); err != nil {
		return err
	}
	...
	if err := cluster2.WaitForAuth(ctx, cluster1.GetClusterID(), 120*time.Second); err != nil {
		return err
	}
