...
func (a *Args) Handler(ctx context.Context) error {
	...
	// Remove proxy cluster 1.
	if err := cluster1.RemoveProxy(ctx, 120*time.Second); err != nil {
		return err
	}

	// Remove proxy cluster 2.
	if err := cluster2.RemoveProxy(ctx, 120*time.Second); err != nil {
		return err
	}
